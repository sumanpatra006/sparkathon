...
export default function DashboardSidebar({
  ...
  const handleLogout = async () => {
    try {
      await api.get("/v1/admin/logout");
    } catch (err) {
      // Optionally handle error, e.g., toast.error('Logout failed')
    }
      ...
      >
        ...
        <nav className="mt-8">
          <div className="px-4 space-y-2">
            ...
            <button
              className="w-full flex items-center space-x-3 px-4 py-3 text-walmart-dark-gray hover:bg-walmart-gray rounded-lg transition-colors mt-2"
